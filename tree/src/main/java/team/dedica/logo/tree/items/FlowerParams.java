...
public class FlowerParams implements GrowthParameters {

    public static final double MIN_DIAMETER_TO_GROW = 0.5;
    public static final double MIN_BRANCH_PROBABILITY = 0.55;
     ...
     */
    public static final float BRANCH_PROBABILITY_FACTOR = 0.3f;

    /**
     * branch growth velocity reduction per cycle
     */
    public static final float VELOCITY_REDUCTION = 0.75f;
    public static final int MAX_STEM_DIAMETER = 10;
    public static final int MIN_ITERATION_TO_BRANCH = 6;

    public PlantSegment getSeed(PVector origin, float scaleFactor) {
        return new PlantSegment(origin,
                new PVector(0, random(5, 10) * -1 * scaleFactor),
                ((random(5, MAX_STEM_DIAMETER)) * scaleFactor),
                scaleFactor,
                0,
                this
        );
    ...
    public boolean canStillGrow(PlantSegment plantSegment) {
    ...
    @Override
    public boolean canBranch(PlantSegment segment) {
    ...
    public PVector createGrowthVector(PlantSegment plant) {
